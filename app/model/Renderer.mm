...
- (id<MTLCommandBuffer>)drawWithPassDescriptor:(MTLRenderPassDescriptor*)passDescriptor {
    ...
    size_t length = vertexData.size() * sizeof(float);
    [encoder setVertexBytes:&vertexData length:length atIndex:0];
    [encoder setVertexBytes:&viewportSize length:8 atIndex:1];
    NSLog(@"%lu", sizeof(simd_uint2));
